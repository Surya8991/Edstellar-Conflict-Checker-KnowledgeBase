      ...
      {turns.map((turn, i) => (
        <Card key={i} className="space-y-3">
          <div className="flex flex-wrap items-center gap-1.5 text-xs text-slate-500">
            ...
            {turn.question ? <span>{turn.question}</span> : <span className="italic">overview of {turn.inputs.length} input{turn.inputs.length === 1 ? "" : "s"}</span>}
          ...
          <div className="rounded-lg border border-indigo-100 bg-indigo-50/50 p-3">
            ...
            {turn.answer.keyFindings?.length > 0 && (
            ...
            {turn.matches.map((m, j) => (
              <div key={j} className="rounded-lg border border-slate-200 p-2.5">
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <span className="ml-auto shrink-0 tabular-nums text-slate-400">{m.groups.length} conflict{m.groups.length === 1 ? "" : "s"}</span>
                </div>
                {m.groups.length === 0 ? (
                      ...
                      <div key={k} className="rounded-md bg-slate-50 p-2">
                        <div className="flex flex-wrap items-center gap-1.5 text-xs">
                          ...
                          <span className="ml-auto tabular-nums text-slate-400">
                            gap {g.positionGap.toFixed(1)} · {g.totalClicks} clicks · {g.totalImpressions.toLocaleString()} impr
                          </span>
                        </div>
