...
Top-level nav: Edstellar Database (the `/edstellar-database` page), Search Console (an expandable parent - each GSC report is its own sub-page at `/search-console?section=<slug>`, ordered by importance: Overview, Almost Page 1 (striking distance, `striking-distance`), Low CTR (`ctr-opportunity`), Content Gaps (semantic - pgvector match against the corpus, `catalog-gap`), Movers, Lost Clicks (`untapped`), Stale Pages, Index Coverage; cannibalization has its own top-level tool, not a GSC sub-page. Labels are plain-language but the URL slugs are unchanged.), Conflict Checker, Content Clusters, Settings. Dashboard, Manager View, Competitors, Bulk Check, Content Audit, Internal Links, Funnel Strategy, and Catalog Conflicts live under the collapsible **Additional Tools** section of the sidebar. (Score History - the old `/history` page - was removed in Session 14; the dashboard shows recent checks inline and editorial-outcome data still powers the Manager View.)
...
| `/keyword-cannibalization` | Queries where 2+ Edstellar pages compete, with severity, the page to keep (primary), and a rule-based action (consolidate / differentiate / monitor). 4 tabs plus an **AI Assistant** tab: paste a batch of URLs/keywords and Groq matches each to the conflicts it appears in (tagged by tab) and writes an analysis, with every turn stored (`cannibalization_chats`). Conflict tabs: near-position / all / cross-type (course vs page) / blogs-to-merge. Shared filter bar: search, severity/action/max-gap **chips**, type/sort **dropdowns**; the max-gap window hides groups whose page spread is too wide. Reads the pre-computed `keyword_conflicts` table (`npm run cannibalization` or gsc-snapshot cron Job 5); the snapshot also writes daily trend history (`keyword_conflicts_history`) and an impression-weighted position variance per group. Sits directly below Search Console. The commercial-intent signal (a blog outranking a course) still boosts severity to high behind the scenes but is never shown as a label - see AGENTS.md. |
| `/competitors` | SERP-based competitor research per topic (needs `SERPER_API_KEY`). Under Additional Tools. |
| `/edstellar-database` | **Edstellar Database** - browse/search the ingested pages by content type; CSV export/import. A **Primary Keyword** column shows each page's #1 GSC query (by clicks, branded terms stripped) with its last-full-month clicks/impressions/position; a **Compact rows** toggle clamps long cells to one line; and a **Columns** menu shows/hides columns (Category + Signals hidden by default). A **Redirect / 404** bucket collects pages that 301/302 redirect elsewhere (`canonical_url`) or return 4xx/5xx (`http_status`) and pulls them out of the normal type counts. Sits directly below Dashboard in the nav. |
