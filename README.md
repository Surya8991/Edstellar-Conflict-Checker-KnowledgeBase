...
| `/corpus` | **Edstellar Database** - browse/search the ingested pages by content type; CSV export/import. A **Redirect / 404** bucket collects pages that 301/302 redirect elsewhere (`canonical_url`) or return 4xx/5xx (`http_status`) and pulls them out of the normal type counts. Sits directly below Dashboard in the nav. |
