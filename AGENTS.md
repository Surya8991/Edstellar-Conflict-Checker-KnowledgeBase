...
- **HTTP-status freshness** (§18M): `lib/http-status.refreshHttpStatus({ limit, concurrency })` is the shared probe→`pages.http_status` writer. The daily `gsc-snapshot` cron runs it as **Job 6 over the WHOLE corpus** (`CRON_HTTP_STATUS_DAILY_LIMIT` default 100000 ≈ all, `CRON_HTTP_STATUS_CONCURRENCY` default 24 to fit the 300s budget, oldest-audited first, LAST + isolated, writes batched every 200). The weekly `audit-links` cron calls the same fn (limit 1500) as a backstop. If the daily sweep ever risks the budget, lower `CRON_HTTP_STATUS_DAILY_LIMIT`. No new cron entry - keep it that way (Hobby 2-cron cap).
