...
- **HTTP-status freshness** (§18M): `lib/http-status.refreshHttpStatus({ limit })` is the shared probe→`pages.http_status` writer. The weekly `audit-links` cron does the full sweep (limit 1500); the daily `gsc-snapshot` cron runs it as **Job 6** (bounded `CRON_HTTP_STATUS_DAILY_LIMIT`, default 600, oldest-audited first, LAST + isolated). No new cron entry - keep it that way (Hobby 2-cron cap).
